...
public class LogicServiceImpl implements LogicService {
	...
	public void add() {
		...
		originalId.stream().forEach(id -> {
			...
			TimeLogs timeLogsPunchIn= timeLogsRepository.findByCreatedAtAndUserIdFkAndIsLogged(strDate,id,1);
			TimeLogs timeLogsPunchOut= timeLogsRepository.findByCreatedAtAndUserIdFkAndIsLogged(strDate,id,0);
			loginDate = timeLogsPunchIn.getCreatedAt();
			logoutDate = timeLogsPunchOut.getCreatedAt();
	...
	public List<AttendenceDTO> listByDayMonth(int year, int month) {
		...
		List<Attendance> attendances1 = attendanceRepository.findAllByCreatedAt(year, month);
		attendances1.stream().forEachOrdered(data -> {
	...
	public DetailsDTO listByDate(int date, int user_id_fk) {
		...
		timeLogs.stream().forEachOrdered(data -> {
